...
class OptionsTestCase extends TestCase
{
    ...
    public function testTimeout(): void
    {
        $options = new Options(
            ...
            Options::DEFAULT_PRIORITY,
            Options::DEFAULT_ATTEMPTS,
            Options::DEFAULT_RETRY_DELAY,
            $expected = 0xDEAD_BEEF
        );

        $this->assertSame($expected, $options->getTimeout());
    }

    public function testTimeoutImmutability(): void
    {
        $original = new Options(
            Options::DEFAULT_DELAY,
            Options::DEFAULT_PRIORITY,
        ...
        $this->assertSame($expected, $original->getTimeout());

        $mutable = $original->withTimeout($expected * 2);

        $this->assertSame($expected, $original->getTimeout());
        $this->assertSame($expected * 2, $mutable->getTimeout());
    }

    public function testTimeoutCreationFromAnotherOne(): void
    {
        $copy = Options::from(
            $original = new Options(
                Options::DEFAULT_DELAY,
                ...
                Options::DEFAULT_ATTEMPTS,
                Options::DEFAULT_RETRY_DELAY,
                $timeout = \random_int(0, \PHP_INT_MAX)
            )
        );

        $this->assertNotSame($original, $copy);

        $this->assertSame($timeout, $original->timeout);
        $this->assertSame($original->timeout, $copy->timeout);
    }

    public function testTimeoutMergingWithDefaults(): void
    {
        $original = new Options(
            Options::DEFAULT_DELAY,
            Options::DEFAULT_PRIORITY,
        ...
        $this->assertSame($timeout, $original->merge(new Options())->getTimeout());
        $this->assertSame($timeout, (new Options())->merge($original)->getTimeout());
    }

    public function testTimeoutMergingByNewestValue(): void
    {
        $defaults = new Options(
            Options::DEFAULT_DELAY,
            Options::DEFAULT_PRIORITY,
        ...
        $this->assertSame($modified->getTimeout(), $defaults->merge($modified)->getTimeout());
    }
